...
void SceneCreator(int floor,int numElevtor, int numPeople, vector<Elevator>& elevators, vector<Person>& people)
{
	...
	for (int i = 0; i < numElevtor; i++)
	{
		...
		//if statment to say what direction will be picking people up in
		if (i == 1)
		{
...
void sortThis(vector <int> &num,vector<int> &id)
{
	int i, j, key ,key2;
	...
	for (j = 1; j < size; j++)
	{
		...
		for (i = j - 1; i >= 0; i--) {
			if (key < num[i]) {
				...
				id[i + 1] = id[i]; //changing something
